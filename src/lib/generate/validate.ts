...
export async function validateGrokRaster(
  ...
  if (countBlackPixels(binary) < 80) {
    return { ok: false, reason: "empty after threshold", visionCost };
  }
  ...
  if (repair.rejected || repair.components !== 1) {
    return { ok: false, reason: `not one piece (components=${repair.components})`, visionCost };
  }

  const rings = hasEndRings(binary);
  if (!rings.ok) {
    return { ok: false, reason: "missing left/right end rings with holes", visionCost };
  }
  ...
  if (hasGrayPixels(data)) {
    return { ok: false, reason: "gray pixels remain", visionCost };
  }
  ...
  return { ok: true, png, svg, transcribed: transcribed.text, visionCost };
}
